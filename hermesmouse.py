...
def to_absolute(x, y):
    ...
    w = w or 1
    h = h or 1
    return int((x - left) * 65535 / w), int((y - top) * 65535 / h)
...
def sweep(cx=None, cy=None, r=220):
    ...
    for deg in range(0, 361, 2):
        ...
        user32.SetCursorPos(int(cx + r * math.cos(rad)),
...
def screenshot(path, region=None):
    ...
    if region is None:
        ...
        vl, vt, vw, vh = region
    if vw <= 0 or vh <= 0:
        return {"error": f"gecersiz bolge: {vw}x{vh}"}
    hdc_screen = hdc_mem = hbmp = None
    try:
        hdc_screen = user32.GetDC(0)
        hdc_mem = gdi32.CreateCompatibleDC(hdc_screen)
        hbmp = gdi32.CreateCompatibleBitmap(hdc_screen, vw, vh)
        gdi32.SelectObject(hdc_mem, hbmp)
        ok = gdi32.BitBlt(hdc_mem, 0, 0, vw, vh, hdc_screen, vl, vt, SRCCOPY)
        if not ok:
        ...
        with open(path, "wb") as f:
            f.write(b"BM")
            f.write((14 + 40 + buf_size).to_bytes(4, "little"))
...
$windowTitle = $params.windowTitle; $elementId = $params.elementId
$searchBy = $params.searchBy; $action = $params.action
...
if (-not $window) {
    ...
    $allWins = $desktop.FindAll([System.Windows.Automation.TreeScope]::Children, $typeProp)
    foreach ($w in $allWins) {
        if ($w.Current.Name -match [regex]::Escape($windowTitle)) { $window = $w; break }
    }
}
if (-not $window) {
...
if ($action -eq 'list') {
    $allElems = $window.FindAll([System.Windows.Automation.TreeScope]::Descendants,
        [System.Windows.Automation.Condition]::TrueCondition)
    $results = @()
    foreach ($e in $allElems) {
        $r = $e.Current.BoundingRectangle
        if ($r -and $r.Width -gt 0 -and $r.Height -gt 0) {
            $results += [ordered]@{name=$e.Current.Name; aid=$e.Current.AutomationId
                ct=$e.Current.ControlType.ProgrammaticName; cls=$e.Current.ClassName
...
$propMap = @{'Name'=[System.Windows.Automation.AutomationElement]::NameProperty
...
if ($searchBy -eq 'ControlType') {
    $ctField = [System.Windows.Automation.ControlType].GetField($elementId, [System.Reflection.BindingFlags]'Public,Static')
    if (-not $ctField) { Write-Output (@{error="Unknown ControlType"; value=$elementId} | ConvertTo-Json -Compress); exit 1 }
    ...
    $elemCond = New-Object System.Windows.Automation.PropertyCondition(
        $propMap[$searchBy], $elementId, [System.Windows.Automation.PropertyConditionFlags]::IgnoreCase)
} else { Write-Output (@{error="Unknown searchBy"; value=$searchBy} | ConvertTo-Json -Compress); exit 1 }
$elem = $window.FindFirst([System.Windows.Automation.TreeScope]::Descendants, $elemCond)
if (-not $elem) {
    $all2 = $window.FindAll([System.Windows.Automation.TreeScope]::Descendants,
        [System.Windows.Automation.Condition]::TrueCondition)
    foreach ($e in $all2) { if ($e.Current.Name -match [regex]::Escape($elementId)) { $elem = $e; break } }
}
if (-not $elem) { Write-Output (@{error="Element not found"; searchBy=$searchBy; id=$elementId} | ConvertTo-Json -Compress); exit 1 }
$r = $elem.Current.BoundingRectangle; $cx = [int]($r.X + $r.Width/2); $cy = [int]($r.Y + $r.Height/2)
Write-Output (@{ok=$true; x=$cx; y=$cy; w=[int]$r.Width; h=[int]$r.Height; name=$elem.Current.Name; aid=$elem.Current.AutomationId} | ConvertTo-Json -Compress)
exit 0
"""
...
def _run_uia(window_title: str, element_id: str = "",
    ...
    params = {"windowTitle": window_title, "elementId": element_id,
              "searchBy": search_by, "action": action}
    tmp_param = tmp_script = None
    raw = ""
    try:
...
_WF_VALID_DO = {"click", "dclick", "rclick", "move", "type", "key",
                "wait", "if_exists", "screenshot", "assert", "repeat"}
...
def run_step(step, index):
    ...
    if do == "assert":
        ...
        if expect == "present":
            ...
            return (not exists,
                    f"assert '{target}': yok (beklenen)" if not exists
        ...
        for raw in f:
            ...
            if do == "if_exists":
                ...
                if len(fields) > 2 and fields[2]:
                    step["by"] = fields[2]
                if thendo.strip():
                    step["then"] = {"do": thendo.strip().lower(), "target": step["target"]}
                steps.append(step)
                continue
            fields = [_strip_q(p) for p in rest.split("|")]
            if do == "type":
    ...
    if cmd == "pos":
        ...
        if action in ("click", "dclick", "rclick") and not is_elevated():
            print("[uyari] Surec yonetici degil; hedef pencere yonetici olarak "
                  "calisiyorsa tiklama etkisiz kalabilir.", file=sys.stderr)
        if action == "click":
            click(x, y)
        elif action == "dclick":
            dclick(x, y)
